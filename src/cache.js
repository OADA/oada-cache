    ...
    function dbUpsert(req) {
      ...
      return db
        ...
        .then(result => {
          ...
          if (result.doc._INCOMPLETE_RESOURCE)
          ...
          if (req.method && req.method.toLowerCase() === "delete") {
            ...
            if (pathLeftover) {
              ...
              var curData = {};
              if (pointer.has(result.doc.doc, pathLeftover))
    ...
    function getResFromDb(req, offline) {
      ...
      return db
        ...
        .then(resource => {
          if (
            !offline &&
            (resource.doc._accessed + expiration <= Date.now() ||
              !resource.doc._valid)
          ) {
    ...
    async function get(req) {
      ...
      if (!/^\/resources/.test(urlObj.path)) {
        ...
        req.url =
          ...
          urlObj.host +
          "/" +
          lookup.doc.resourceId +
          lookup.doc.pathLeftover;
      }
