	...
	for (let i = 0; i < 4; i++) {
		describe(`Testing connection ${i+1}`, async function() {
			...
			it(`Should error when neither 'url' nor 'path' are supplied`, async function() {
				console.log(`Cache: ${connections[i].cache ? true: false}; Websocket: ${connections[i].websocket}`)
				try {
			...
			it(`Should produce a 412 if two PUTs are executed in parallel with the same 'If-Match' header`, async function() {
				...
				try {
					var result = await Promise.join(putOne,putTwo, async function(putOne,putTwo) {
						expect(putOne.status).to.equal(412)
						expect(putTwo.status).to.equal(412)
					})
				} catch (err) {
					expect(err.response.status).to.equal(412)
				}
